...
# 질문 단계
def step1():
    st.markdown("<h2 style='text-align: center; color: #FF5733;'>1/3 질문</h2>", unsafe_allow_html=True)
    st.markdown("### 선호하는 플레이 스타일을 선택하세요.")
    st.session_state.q1 = st.radio("1. 선호하는 플레이 스타일은?", ["공격적", "수비적"], index=0, horizontal=True)
    st.button("다음 ➡️", on_click=go_next_step)

def step2():
    st.markdown("<h2 style='text-align: center; color: #FF5733;'>2/3 질문</h2>", unsafe_allow_html=True)
    st.markdown("### 게임할 때 당신은?")
...
def step3():
    st.markdown("<h2 style='text-align: center; color: #FF5733;'>3/3 질문</h2>", unsafe_allow_html=True)
    st.markdown("### 당신의 성향에 가까운 것은?")
    st.session_state.q3 = st.radio("3. 당신의 성향에 가까운 것은?", ["초반 스노우볼형", "후반 밸류형"])
    st.button("결과 보기 🏆", on_click=go_next_step)

def result():
    st.markdown("<h2 style='text-align: center; color: #33C3FF;'>🏆 추천 챔피언</h2>", unsafe_allow_html=True)
    ...
    st.markdown(f"### {champion}", unsafe_allow_html=True)
    st.info(desc)
