...
def step1():
    st.markdown("## 1/3 질문")
    st.markdown("### ❤️❤️❤️ 선호하는 플레이 스타일을 선택하세요 ❤️❤️❤️")
...
def step3():
    ...
    st.session_state.q3 = st.radio("", ["💘 초반 스노우볼형 💘", "💖 후반 밸류형 💖"])
    st.button("결과 보기 🏆", on_click=go_next_step)

def result():
    st.markdown("## 🏆 추천 챔피언")
    key = (st.session_state.q2.replace("💗","").replace("💓","").replace("💞","").replace("💕","").replace("💖","").strip(),
           st.session_state.q1.replace("❤️","").replace("💖","").strip(),
           st.session_state.q3.replace("💘","").replace("💖","").strip())
